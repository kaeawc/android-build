...
| 9a | Reduce Artifact Swap publish CI tax | **in progress** | `ci-should-publish.sh` skips publishing when only non-swap-relevant files changed and leaves `artifact-swap-green-main` unchanged; `setup-gradle` caches the distribution and dependency caches for the two CLI-driven Gradle invocations. Pipeline 165 s -> 41 s on the first cached run; skip path not yet exercised; the advance push uses `GREEN_MAIN_TOKEN` (PAT) when a newer workflow edit has already landed on main, falling back to `GITHUB_TOKEN` otherwise |
